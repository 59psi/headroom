...
- `src/headroom/routes/` — `health`, `public` (unauthenticated branding logo for the login page), `auth` (setup/login/logout/me/token/passkeys), `cases`, `hats`, `import_jobs`, `rooms`, `search`, `meta`, `settings`, `admin`, `share`, `share_links` (mgmt + `/api/public/share/{token}` read-only view with token-gated photo streaming). `admin` is a **package** (`routes/admin/`) — `errors`, `backups`, `activity`, `reports`, `ebay`, `catalog`, each a prefix-less router; `__init__.py` applies the `/api/admin` prefix, tag and `require_admin` once, so a submodule can't ship an unguarded admin route
