...
## Melin case rack — [`melin-rack-v3.zip`](melin-rack-v3.zip)
...
> and adjust `case_w` if it differs.

### What's inside

Everything extracts to a `melin-rack-v3/` folder:
...
~241 × 258 mm footprint (~167 mm tall with pegs) overhangs a standard
...
- Print in the modeled orientation (floor on bed). **Zero supports needed** —
  every feature is vertical, chamfered, ramped, or a short bridge (the socket
  pockets bridge ~10 mm at z ≈ 6.5 mm, trivial for the H2D).
