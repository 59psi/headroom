  ...
  return (
    <nav className="bottom-nav d-lg-none">
      ...
      <NavLink to="/settings" className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} style={{ position: 'relative' }}>
        ...
        <span>Settings</span>
        {errors > 0 && (
          /* Labelled, because a bare red dot is unreadable to a screen reader
             and ambiguous to everyone else — it counts hats whose ANALYSIS
             failed, not errors in general. */
          <span
            role="status"
            aria-label={`${errors} hat${errors === 1 ? '' : 's'} failed analysis`}
            title={`${errors} hat${errors === 1 ? '' : 's'} failed analysis`}
            style={{
