/**
 * The colour-search result list is ordered by the server on a score the UI
 * never shows, so without a label the ordering reads as broken: a hat whose
 * accent is EXACTLY your colour displays Δ0 and still sits below a hat at Δ5.
...
describe('matchedRankLabel', () => {
  it('says nothing for a hat matched on its main colour', () => {
  ...
  it('distinguishes a secondary colour from a deeper accent', () => {
    expect(matchedRankLabel(2)).toBe('secondary');
    expect(matchedRankLabel(3)).toBe('accent');
  });

  it('calls anything past the third colour an accent too', () => {
    // Colours are capped at three by the analyser but not by the manual
